...
//      }
        ...
        bw = new BufferedWriter(new FileWriter(prjFile));
        if (wctRaster.isNative() && ! (wctRaster instanceof GridDatasetRemappedRaster)) {
...
//        }
    ...
    throws WCTExportNoDataException, IOException, InvalidRangeException {
        ...
        NetcdfFileWriter ncfile = NetcdfFileWriter.createNew(f.toString(), true);
        ...
        List<ucar.nc2.Dimension> dims = new ArrayList<ucar.nc2.Dimension>();
...
//        System.out.println("exporting variable '"+wctRaster.getVariableName()+"' to CF-Gridded NetCDF");
        ncfile.addVariable(wctRaster.getVariableName(), ucar.ma2.DataType.DOUBLE, dims);
        ...
        List<ucar.nc2.Dimension> latDims = new ArrayList<ucar.nc2.Dimension>();
        latDims.add(latDim);
        ncfile.addVariable("lat", ucar.ma2.DataType.DOUBLE, latDims);
        ...
        List<ucar.nc2.Dimension> lonDims = new ArrayList<ucar.nc2.Dimension>();
        lonDims.add(lonDim);
        ncfile.addVariable("lon", ucar.ma2.DataType.DOUBLE, lonDims);
        ...
        if (isTimeDimensionNeeded) {
            // int time(time) ;
            // time:units = "hours" ;
            List<ucar.nc2.Dimension> timeDims = new ArrayList<ucar.nc2.Dimension>();
            timeDims.add(timeDim);
            ncfile.addVariable("time", ucar.ma2.DataType.INT, timeDims);
...
//        	System.out.println((bounds.x + n*cellWidth) + " " +cellWidth+" , "+bounds);
        ...
        if (isTimeDimensionNeeded) {
        	ucar.ma2.ArrayInt timeArray = new ucar.ma2.ArrayInt.D1(timeDim.getLength(), false);
...
//        System.out.println("exporting variable '"+wctRaster.getVariableName()+"' to CF-Gridded NetCDF");
        ...
        // float height(height) ;
        // lon:units = "meters" ;
        List<ucar.nc2.Dimension> heightDims = new ArrayList<ucar.nc2.Dimension>();
        heightDims.add(heightDim);
        ncfile.addVariable("height", ucar.ma2.DataType.DOUBLE, heightDims);
